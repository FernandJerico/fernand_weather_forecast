...
class WeatherRemoteDatasource {
  ...
  Future<Either<String, GetDetailWeatherResponseModel>> getDetailWeather(
      double latitude, double longitude) async {
    final url = Uri.parse(
        'https://api.tomorrow.io/v4/weather/forecast?location=$latitude,$longitude&apikey=${Variables.apiKeyTomorrow}');

    final response = await http.get(url);

    if (response.statusCode == 200) {
      return Right(GetDetailWeatherResponseModel.fromJson(response.body));
    } else {
      try {
        final responseBody = json.decode(response.body);
        var message = responseBody['message'];
  ...
  Future<Either<String, GetDetailWeatherResponseModel>> getDetailWeatherByName(
      String cityName) async {
    final url = Uri.parse(
        'https://api.tomorrow.io/v4/weather/forecast?location=$cityName&apikey=${Variables.apiKeyTomorrow}');
